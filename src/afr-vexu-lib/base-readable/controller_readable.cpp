...
namespace AFR::VexU::BaseReadable{
    error_t controller_button_readable::update_private(const double& delta_seconds){
    ...
    controller_button_readable::controller_button_readable(const AFR::VexU::scheduled_update_t& update_period,
    ...
    error_t controller_button_readable::digital_is_pressed(const pros::controller_digital_e_t& button, bool& result){
        result = digital_map_.at(button);
        return SUCCESS;
    }

    error_t controller_button_readable::get_analog_value(const pros::controller_analog_e_t& analog, int32_t& result){
    ...
    namespace Controller{
        error_t init(){
            driver_controller = new controller_button_readable{driver_update_period, pros::E_CONTROLLER_MASTER};
            operator_controller = new controller_button_readable{operator_update_period, pros::E_CONTROLLER_PARTNER};
            return SUCCESS;
        }
