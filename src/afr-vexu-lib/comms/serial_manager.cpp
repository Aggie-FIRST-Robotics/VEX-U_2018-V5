...
namespace AFR::VexU {
	...
	serial_manager* serial = nullptr;

	void init_serial_manager(const scheduled_update_t& update_period){
		serial = new serial_manager(update_period);
	}
	...
	serial_manager::serial_manager (const scheduled_update_t& update_period) : scheduled(update_period, "Serial Manager"), nameable("Serial Manager") {
		...
		pros::c::serctl(DEVCTL_SET_BAUDRATE, (void*)115200);
	...
	void serial_manager::update_private(double delta_seconds) {
		handle_read();
