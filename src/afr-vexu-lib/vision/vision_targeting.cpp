...
namespace AFR::VexU::Vision {
    ...
    void vision_targeting::set_value_private(encoder_tuple enc_vals, double delta_seconds) {
        ...
        short rect_num = serial_manager->odroid_table.read(FRAME_NUM_ADDR);
        if(rect_num > 0 && serial_manager->odroid_table.card(FRAME_NUM_ADDR) <= UPDATE_RATE) {
            for(uint8_t i = 0; i < rect_num; i++) {
                rectangle serial_rect;
                serial_rect.x = serial_manager->odroid_table.read(2+4*i);
                serial_rect.y = serial_manager->odroid_table.read(3+4*i);
                serial_rect.width = serial_manager->odroid_table.read(4+4*i);
                serial_rect.height = serial_manager->odroid_table.read(5+4*i);
