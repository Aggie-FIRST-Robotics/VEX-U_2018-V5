...
namespace AFR::VexU::Robot2{
    ...
    void init_auto(){
        ...
        drive_right_pid = new pid_action<double, int16_t>{10, drive_right, 100, 0, 0, -12000, 12000, -6000, 6000, 0,
                                                          drive_right_encoder, 0, "drive_right_pid"};
        drive_left_pid = new pid_action<double, int16_t>{10, drive_left, 100, 0, 0, -12000, 12000, -6000, 6000, 0,
                                                         drive_left_encoder, 0, "drive_left_pid"};
        ...
        drive_forward = auto_state_con->add_state(
                suck_actions,
                [](state* last_state) -> void{
                    drive_right_pid->set_target(DRIVE_FORWARD_TARGET);
                    drive_left_pid->set_target(DRIVE_FORWARD_TARGET);
                    Cap::elbow_pid_action->set_target(Cap::DOWN_ELBOW_TARGET);
                },
                "drive_forward",
                []() -> state*{
                    if(drive_right_pid->is_in_range(50) && drive_left_pid->is_in_range(50)){
        ...
        drive_backward = auto_state_con->add_state(
                pid_actions,
                [](state* last_state) -> void{
                    drive_right_pid->set_target(0);
                    drive_left_pid->set_target(0);
